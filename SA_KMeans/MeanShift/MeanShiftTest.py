...
def main():
    X, _ = make_blobs(n_samples=100, centers=3,random_state=0, cluster_std=0.4)

    mean_shifter = MeanShift()
    shift_points, mean_shift_result = mean_shifter.fit(X, kernel_bandwidth=0.5)

    np.set_printoptions(precision=3)
    print('input: {}'.format(X))
    print('assined clusters: {}'.format(mean_shift_result))
    print('shift points: {}'.format(shift_points))
